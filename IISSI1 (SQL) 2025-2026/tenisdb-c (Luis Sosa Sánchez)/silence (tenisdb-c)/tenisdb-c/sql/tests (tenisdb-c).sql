...
-- ---------------------------------------------------------------------------- 
DELIMITER //
CREATE OR REPLACE PROCEDURE p_test_max_2_players_for_1_trainer()
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        CALL p_log_test('RA-02', 'RA-02: Un entrenador no puede entrenar a más de 2 tenistas simultáneamente', 'PASS');

    CALL p_populate_db();
    -- El entrenador 19 ya entrena a 2 jugadores, añado que entrena a otro más 
    INSERT INTO players (player_id, trainer_id, ranking, activo) VALUES (100, 19, 11, TRUE);
    
    -- Si llega aquí, significa que NO saltó la excepción → FAIL
    CALL p_log_test('RA-02', 'ERROR: Se permitió que un entrenador entrene a más de 2 tenistas simultáneamente', 'FAIL');
END //
DELIMITER ;
...
BEGIN
    ...
    CALL p_test_sets_max_5_sets();
	 CALL p_test_max_2_players_for_1_trainer();
