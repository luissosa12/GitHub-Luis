...
ÚLTIMA MODIFICACIóN: 10/01/2019
...
def lee_contratos(fichero):
    ...
    with open(fichero, encoding='utf8') as f:
        lector = csv.reader(f)
        next(lector) # Para saltar la cabecera
        contratos=[]
...
def lee_contratos2(fichero):
    '''Versión con dictreader y namedtuple'''
    with open(fichero, encoding='utf8') as f:
        lector = csv.DictReader(f)
        contratos=[]
        for registro in lector:
            numSS = registro["Numero SS"]
            fecha = datetime.strptime(registro["fecha"], '%d/%m/%Y').date()
            codigo = registro["codigo"]
            dias = int(registro["dias"])
            hora = int (registro["horas"])
            sueldo = float(registro["sueldo"])
            c = Contrato(numSS, fecha, codigo, dias, hora, sueldo)
            contratos.append(c)
    return contratos

def lee_contratos3(fichero):
    '''version 3 con csv.reader, namedtuple y listas por compresion'''
    with open(fichero, encoding='utf8') as f:
        lector = csv.reader(f)
        next(lector) # Para saltar la cabecera
        contratos=[ Contrato(numSS, datetime.strptime(fecha, '%d/%m/%Y').date(), 
                             codigo, dias, int (hora), float(sueldo)) 
                   for numSS, fecha, codigo, dias, hora, sueldo in lector]
    return contratos
...
def acumuladorDiasPorNumSS(contratos):
    ...
    '''ENTRADA: Una lista de contratos list(Contrato)
       SALIDA: Un diccionario str:int en el que las claves son los números de la
       seguridad social de los trabajadores, y los valores, el número total de días
       que ha trabajado ese trabajador''' 
    res = dict()
    for contrato in contratos:
        ...
        if clave in res:
            ...
            res[clave] = contratos.dias
    return res

def acumuladorDiasPorNumSS2(contratos):
...
def muestra_evolucion_contratos(contratos, codigo):
    ...
    '''
    #genero un diccionario con el conteo 
    dicc_cont = contador_por_anyos(contratos, codigo)
    #no puedo usar las claves del diccionario porque puede haber algún año
    #en el que no haya ningún contrato de la actividad buscada, por lo que
    #necesito un recorrido para extraer todo los años registrados en los datos
    anyos = sorted({contrato.fecha.year  for contrato  in contratos})
    numero_contratos = [dicc_cont[a] for a in anyos]
    
    plt.title("Número de contratos por anyo de la actividad " + codigo)
    plt.bar(range(len(numero_contratos)), numero_contratos, tick_label=anyos)
    plt.show()
    

def contador_por_anyos(contratos, codigo):
    ''' Función auxiliar: versión 1 (con Counter)'''
    ''' Recibe una lista de tuplas con la información sobre los contratos y
    el código de una actividad, y genera un diccionario contador int:int, donde
    las claves son los años de los contratos y los valores el número total de
    contratos con el código de actividad dado como parámetro de ese año'''
    res = Counter()
    anyos= [contrato.fecha.year for contrato in contratos if contrato.codigo == codigo]
    res.update(anyos)
    return res 

def contador_por_anyos2(contratos, codigo):
    ''' Función auxiliar: versión 2 (con defaultdict)'''
    ''' Recibe una lista de tuplas con la información sobre los contratos y
    el código de una actividad, y genera un defaultdict int:int, donde
    las claves son los años de los contratos y los valores el número total de
    contratos con el código de actividad dado como parámetro de ese año'''
    res = defaultdict(lambda:0)
    for contrato in contratos:
        if contrato.codigo == codigo:
            clave = contrato.fecha.year
