...
def lee_escrutinio(fichero_votos, fichero_escaños):
    ...
    '''
    with open(fichero_votos, 'r', encoding='utf-8') as f:
        lector = csv.reader(f)
        next(lector)
        votos = [(prov,part,int(vot)) for prov,part,vot in lector]

    with open(fichero_escaños, 'r', encoding='utf-8') as f:
        lector = csv.reader(f)
        next(lector)
        escaños = {prov:int(num) for prov,num in lector}
    return votos,escaños
...
def calcula_provincias(votos):
    ...
    '''
    provincias = set([prov for prov,_,_ in votos])
    return provincias
...
def calcula_partidos(votos):
    ...
    '''
    partidos = set([part for _,part,_ in votos])
    return partidos
...
def calcula_diccionario_provincia(votos, provincia):
    ...
    '''
    diccio = {}
    for prov,part,vot in votos:
        if prov == provincia:
            diccio[part] = vot
    #diccio = {part:vot for prov,part,vot in votos if prov==provincia}
    return diccio
...
def calcula_diccionario_provincias(votos):
    ...
    '''
    datos={}
    provi = calcula_provincias(votos)
    for prov in provi:
        datos[prov] = calcula_diccionario_provincia(votos, prov)
    #return {prov:calcula_diccionario_provincia(votos, prov) for prov in calcula_provincias(votos)}
    return datos
...
def totales_por_partido(tabla_valores):
    ...
    for provincia in tabla_valores:
        counter_part = counter_part + Counter(tabla_valores[provincia])
    return dict(counter_part)
...
def genera_diagrama_tarta(diccionario, limite=None):
    ...
    '''
    lista = [(diccionario[clave],clave) for clave in diccionario]
    paleta = ['blue', 'red', 'green', 'yellow', 'olive', 'orange', 'cyan', 'pink', 'grey']
    lista.sort(reverse=True)
    claves = [clave for _,clave in lista]
    if limite is not None:
        claves = claves[:limite]
    #valores = [diccionario[clave] for clave in claves]
    valores = [valor for valor,_ in lista]
    if limite is not None:
        valores = valores[:limite]
    colores = [paleta[i%len(paleta)] for i in range(len(claves))]
    plt.pie(valores, labels=claves, colors=colores, autopct='%1.1f%%', startangle=140)
    plt.axis('equal')
    plt.show()
    pass
...
def genera_mapa_calor(tabla_valores, limite_columnas=None, fmt='d'):
    ...
    Donde 'filas' es una lista de vectores, uno por cada provincia:. 
        ...
        - Los partidos se ordenarán siempre de la misma forma para los vectores de todas
          las provincias.
    '''
    provincias = sorted(list(tabla_valores.keys()))
    valores = totales_por_partido(tabla_valores)
    lista_partidos =[(valores[partido],partido) for partido in valores]
    lista_partidos.sort(reverse=True)
    partidos = [partido for _,partido in lista_partidos]
    if limite_columnas is not None:
        partidos = partidos[:limite_columnas]
    filas = []
    for provincia in provincias:
        valores_provincia = tabla_valores[provincia]
        fila=[valores_provincia.get(partido,0) for partido in partidos]
        filas.append(fila)
