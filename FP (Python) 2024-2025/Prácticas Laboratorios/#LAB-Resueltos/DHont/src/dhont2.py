...
def lee_escrutinio(fichero_votos, fichero_escaños):
    ...
    with open(fichero_votos, 'r', encoding="utf-8") as f:
        lector = csv.reader(f)
        next(lector)
        votos = [(provin, partido, int(votos)) for provin, partido, votos in lector]
    with open(fichero_escaños, 'r', encoding="utf-8") as f:
        lector=csv.reader(f)
        next(lector)
        escaños = {provin:int(esca) for provin,esca in lector}
...
def calcula_partidos(votos):
    ...
    '''
    partidos = set([partido for _,partido,_ in votos])
    return partidos
