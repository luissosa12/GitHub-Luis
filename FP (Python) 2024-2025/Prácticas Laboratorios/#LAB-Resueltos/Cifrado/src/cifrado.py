...
REVISOR: Fermín Cruz
ÚLTIMA MODIFICACIÓN: 08/11/2018
...
def construye_diccionario_codificacion(k):
    ...
    dicc1 = {min[i] : min[(i + k)%len(min)] for i in range(len(min))}
    dicc2 = {may[i] : may[(i + k)%len(may)] for i in range(len(may))}
    dicc.update(dicc1)
    dicc.update(dicc2)
    '''
    dicc=dict()
    ...
    for i in range(len(min)):
        dicc[min[i]] = min[(i+k)%len(min)]
    for i in range(len(may)):
        dicc[may[i]] = may[(i+k)%len(may)]
    dicc.update(dicc1)
    dicc.update(dicc2)
    '''
    return dicc
...
def cifra_mensaje(texto, k):
    ...
    '''
    cod = construye_diccionario_codificacion(k)
    pala_cod = list()
    for c in texto:
        pala_cod.append(cod.get(c, c))
    return "".join(pala_cod)
...
def descifra_mensaje(texto, k):
    ...
    '''
    cod = construye_diccionario_codificacion(k)
    descod = {valor : clave for clave, valor in cod.items()}
    pala_cod = [descod.get(c, c) for c in texto]
    '''
    return "".join(pala_cod)
...
def cuenta_palabras_validas(texto, vocabulario):
    ...
    tokens = texto.split()
    for token in tokens:
        if (es_palabra_valida(token, vocabulario)):
            cont += 1
    return cont
    #palabras = [token for token in texto.split() if es_palabra_valida(token, vocabulario)]
    #return len(palabras)
...
def rompe_codigo_cifrado(texto, vocabulario):
    ...
    '''
    posibles_desplazamientos = range(len(string.ascii_lowercase))
    aciertos = {i:cuenta_palabras_validas(descifra_mensaje(texto, i), vocabulario) for i in posibles_desplazamientos}
    cremallera = zip(aciertos.values(), aciertos.keys())
    _, mejor_desplazamiento = max(cremallera)
    return mejor_desplazamiento,descifra_mensaje(texto, mejor_desplazamiento)
