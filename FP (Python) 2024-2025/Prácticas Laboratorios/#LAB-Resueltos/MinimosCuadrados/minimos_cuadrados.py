...
import csv
from numba.cuda.cudadrv.enums import CU_DEVICE_ATTRIBUTE_MAX_MAX_TEXTURE_2D_MIPMAPPED_HEIGHT
...
def lee_registros(fichero):
    ...
    lector = csv.reader(f)
    registros = []
    for gen, edad, altura, peso in lector: 
        registros.append((gen, float(edad), float(altura), float(peso)))
...
def pesos_alturas_por_edades(registros, edad_min, edad_max):
    ...
    '''
    pesos_edades =[]
    for _, edad, altura, peso in registros:
        if edad>=edad_min and edad<=edad_max:
            pesos_edades.append((peso,altura))
...
def pesos_alturas_por_genero(registros, genero):
    ...
    '''
    pesos_genero =[]
    for gen, _, altura, peso in registros:
        if gen == genero:
            pesos_genero.append((peso,altura))
...
def muestra_recta_y_puntos(a, b, puntos):
    ...
    x_min,x_max = x_min - x_min*0.1, x_max - x_max*0.1
    y_min,y_max = x_min*a + b, x_max*a + b
