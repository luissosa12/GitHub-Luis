...
#---------------------------------------------------------------------------------------------------------------------------------------------
# OBSERVACIONES:  
#   • Si una vivienda no tiene mejoras, se almacenará una  lista vacía en el campo mejora 
#       de la tupla correspondiente. 
#   • La lista de mejoras de una vivienda no está ordenada por fechas. 
#   • El  valor  del  metro  cuadrado  de  una  vivienda  se  calcula  dividiendo  el  precio  de 
#       compra por los metros cuadrados construidos. 
#   • La cadena de formato para parsear las fechas es "%d/%m/%Y". 
#   • El  número  de  días  entre  dos  fechas,  fecha_inicio  y  fecha_fin,  se  calcula  como 
...
def total_mejoras_por_calle (viviendas: List[Vivienda], par_impar: str) -> Dict[str,int]: 
    ...
    for v in viviendas:
        ...
        if (par_impar == "impar") and (v.numero % 2 != 0):
            res[v.calle] += len(v.mejoras)
    ...
    for v in viviendas:
        for m in v.mejoras:
            ...
            if dias_min is None or dias < dias_min:
                dias_min = dias
                res = (v.propietario, v.calle, v.numero, dias, m.denominacion)
...
def calle_mayor_diferencia_precios(viviendas: List[Vivienda]) -> str: 
    ...
    for v in viviendas:
        ...
        if (v.numero % 2 == 0):         #par -> se resta
            res[v.calle] -= v.precio
    
    mayor_diferencia = max(res.items(), key=lambda x:abs(x[1])) #valor absoluto de los valores; dentro de la Tupla -> (clave, valor)

    return mayor_diferencia[0]
...
def calle_mayor_diferencia_precios2(viviendas: List[Vivienda]) -> str: 
    ...
    return max(res, key=lambda x:abs(res.get(x))) #x:abs(res.get(x)) → obtengo el valor absoluto de cada valor del diccionario
...
def n_viviendas_top_valoradas_por_calle(viviendas: List[Vivienda], fecha: date|None = None, n: int = 3) -> Dict[str, List[Tuple[str,int,int]]]:
    ...
    for v in viviendas:
        if (fecha is None or v.fecha_adquisicion >= fecha):
            ...
            res[v.calle].append((v.propietario, v.numero, valor))
    
    res2 = dict()
    for calle, valor in res.items():
        lista_ordenada = sorted(valor, key=lambda x:x[2], reverse=True)[:n]
        res2[calle] = lista_ordenada
    
    return res2

#EXPLICACIÓN: 
#| Objetivo                            | Código correcto                                             |
#| ----------------------------------- | ----------------------------------------------------------- |
#| Calcular valor total de la vivienda | `v.precio + sum(m.coste for m in v.mejoras)`                |
#| Analizar mejoras una a una          | `for m in v.mejoras: ...`                                   |
#| ❌ Mal uso de `sum(m.coste)`         | Incorrecto: `m.coste` ya es un número, no necesitas `sum()` |

#EXPLICACIÓN 2: 
#| ¿Qué necesitas hacer?                           | ¿Cómo hacerlo?                      |
#| ----------------------------------------------- | ----------------------------------- |
#| Calcular la suma total de costes de mejoras     | `sum(m.coste for m in v.mejoras)`   |
#| Revisar o filtrar mejoras una a una             | `for m in v.mejoras:`               |
#| Comparar fechas, tipos o denominaciones         | `for m in v.mejoras:` + condicional |
#| Nunca usar `sum(m.coste)` si `m.coste` es `int` | ❌ `sum(m.coste)` es un error       |
