...
def lee_viviendas(ruta_fichero: str) -> List[Vivienda]: 
    ...
    with open(ruta_fichero, 'r', encoding='utf-8') as f:
        ...
        next(lector)
        for nombre, calle, numero, fecha_adquisicion, metros, precio, mejoras in lector:
            ...
            tupla = Vivienda(nombre, calle, fecha_adquisicion, numero, metros, precio, mejoras)
            res.append(tupla)
...
def vivienda_con_mejora_mas_rapida(viviendas: list[Vivienda]) -> tuple[str, str, int, int, str]:
    ...
    for v in viviendas:
        if len(v.mejoras) == 0:
            continue  # ignorar viviendas sin mejoras
...
def calle_mayor_diferencia_precios(viviendas: List[Vivienda]) -> str:
    ...
    return max(res, key=lambda calle: abs(res.get(calle)))
