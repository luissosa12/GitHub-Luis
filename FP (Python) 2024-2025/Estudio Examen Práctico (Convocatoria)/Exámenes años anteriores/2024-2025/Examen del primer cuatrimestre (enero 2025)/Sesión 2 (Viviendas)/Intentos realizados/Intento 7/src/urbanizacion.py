...
def parsea_list_mejoras(cadena: str) -> List[Mejora]: 
    if len(cadena.strip()) > 0:
        trozos = cadena.strip().split("*")
        return [parsea_mejora(trocito.strip()) for trocito in trozos]
    else:
        return []
...
def calle_mayor_diferencia_precios(viviendas: List[Vivienda]) -> str: 
    ...
    for v in viviendas:
        ...
        if (v.numero % 2 == 0):
            res[v.calle] -= v.precio            #par -> se resta
    
    mayor_diferencia = max(res.items(), key=lambda x:abs(res.get(x)))
...
def calle_mayor_diferencia_precios(viviendas: List[Vivienda]) -> str: 
    ...
    mayor_diferencia = max(res.items(), key=lambda x:abs(x[1]))
...
def n_viviendas_top_valoradas_por_calle(viviendas: List[Vivienda], fecha: date|None = None, n: int = 3) -> Dict[str, List[Tuple[str,int,int]]]:
    ...
    res2 = dict()
    for calle, valores in res.items():
        mas_valoradas = sorted(valores, key=lambda X:X[2], reverse=True)[:n]
        res2[calle] = mas_valoradas
