...
def parsea_list_addons(cadena: str) -> List[str]: 
    if len(cadena.strip()) > 0:
        ...
        return [trocito.strip() for trocito in trozos]
    else:
        return []
...
def plan_mas_perfiles(suscripciones: List[Suscripcion], fecha_ini: date|None = None,  fecha_fin: date|None = None) -> Tuple[str, int]: 
    ...
    max_plan = max(res.items(), key=lambda x:x[1])
...
def media_dias_por_plan(suscripciones: List[Suscripcion]) -> Dict[str, float]: 
    ...
    for s in suscripciones:
        if s.fecha_fin is not None:
            duracion = (s.fecha_fin - s.fecha_inicio).days
            res[s.tipo_plan].append(duracion)
        
    res2 = dict()
    for tipo, duracion in res.items():
        res2[tipo] = (sum(duracion) / len(duracion))
...
def addon_mas_popular_por_año(suscripciones: List[Suscripcion]) -> Dict[int, str]: 
    ...
    for s in suscripciones:
        ...
        res[anyo].update(s.addons)  # Contamos los addons por año

    res2 = dict()
    for anyo1, addons in res.items():
        mas_veces = max(addons.items(), key=lambda x: x[1])  # Tupla (addon, veces)
        res2[anyo1] = mas_veces[0]  # Guardamos solo el nombre del más frecuente

    return res2

#OTRA FORMA: 
def addon_mas_popular_por_año2(suscripciones: List[Suscripcion]) -> Dict[int,str]: 
    res = defaultdict(Counter)
    
    for s in suscripciones:
        año = s.fecha_inicio.year
        res[año].update(s.addons)

    # Elegimos el más común (posición 0) para cada año
    return {año: addons.most_common(1)[0][0] for año, addons in res.items()}

#most_common(1) → devuelve una lista con el elemento más frecuente.

#[0] → accede a esa única tupla → (addon, frecuencia).

#[0][0] → accede al nombre del addon más popular.
