...
def suscripciones_mas_rentables(suscripciones: List[Suscripcion], n: int = 3, tipos_plan: Set[str]|None = None) -> List[Tuple[str, float]]: 
    ...
    for s in suscripciones:
        if (tipos_plan is None or s.tipo_plan in tipos_plan):
            ...
            duracion = (fecha_fin - s.fecha_inicio).days
            importe_total = (s.precio_mensual / 30) * duracion
            res[s.dni] = importe_total
    
    n_mas_rentables = sorted(res.items(), key=lambda x:x[1], reverse=True)[:n]

    return list(n_mas_rentables)
...
def media_dias_por_plan(suscripciones: List[Suscripcion]) -> Dict[str, float]: 
    ...
    res2 = dict()
    for tipo_plan, duracion in res.items():
        media_dias = (sum(duracion) / len(duracion))
...
def addon_mas_popular_por_año(suscripciones: List[Suscripcion]) -> Dict[int,str]: 
    ...
    for anyo, addons in res.items():
        lista_ordenada = max(addons.items(), key=lambda x:x[1])
        res2[anyo] = lista_ordenada[0]
    return res2
...
def addon_mas_popular_por_año(suscripciones: List[Suscripcion]) -> Dict[int, str]: 
    ...
    for anyo, addons in res.items():
        lista_ordenada = sorted(addons.items(), key=lambda x: x[1], reverse=True)

        # Guardamos solo el nombre del más frecuente
        res2[anyo] = lista_ordenada[0][0]
